          ...
          <div className="flex-1 flex flex-col gap-3 p-5">
            ...
            <div className="flex items-center gap-3 border border-gray-200 rounded-xl px-4 py-3">
              ...
              <div className="w-10 h-10 rounded-full bg-orange-200 flex items-center justify-center text-orange-600 font-bold text-base overflow-hidden flex-shrink-0">
                {order.product?.seller?.avatarUrl ? (
                  <img
                    src={order.product.seller.avatarUrl}
                    alt=""
                    className="w-full h-full object-cover"
                  />
                ) : (
                  order.product?.seller?.username?.[0]?.toUpperCase() ?? '?'
                )}
              </div>
            ...
            <div className="flex border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="flex-1 flex flex-col justify-center px-4 py-3">
                ...
                <span className="text-sm text-gray-500 mt-0.5">
                  {formatTime(order.product?.preparationTime)}
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 border border-gray-200 rounded-xl p-4">
              ...
              <p className={`text-sm leading-relaxed line-clamp-6 ${!order.product?.description ? 'text-gray-400 italic' : 'text-gray-600'}`}>
                {order.product?.description || 'No description for the moment'}
              </p>
            </div>

            {/* Price · Quantity · Total (Grid for better spacing) */}
